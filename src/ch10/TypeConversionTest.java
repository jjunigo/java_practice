...
/*
		...
		*/ //-> 정상
		...
		System.out.println(bNum);//큰 수가 작은수로 들어가서 값이 이상함
		
		double dNum = 3.14;
		int inum = (int) dNum;
		
		System.out.println(inum); // 강제적으로 int 사용으로 소숫점이 사라져서 3만 출력
		 */
		
		double dNum = 1.2;
		float fNum = 0.9F;
		
		int iNum1 = (int)dNum + (int)fNum; //int먼저 변환이 되어 1
		int iNum2 = (int)(dNum + fNum); //숫자먼저 계산후 int로 형변환 되어 2
